...
# Fix parameters of the simulation model
sim_model <- sim_model_sbn_bn(K=cl, seed = NULL)
saveRDS(sim_model, paste0("m3_parameters", cl, ".rds"))
# sim_model <- sim_model_mixture_bn(K=cl, seed = NULL)
# saveRDS(sim_model, paste0("m4_parameters", cl, ".rds"))
print(sim_model)
  ...
  foreach(s = size, .combine = "c") %do% {
    ...
    ari_results <-
      foreach(l = icount(10), .combine = "rbind") %dopar% {
        ...
        # K PROTOTYPES
        kprot_df <- clustMixType::kproto(data.frame(con_df, cat_df, stringsAsFactors = TRUE),
                                         k = cl,
        ...
        # PDQ
        pdq_df <- FPDclustering::PDQ(x=data.frame(con_df, cat_df, stringsAsFactors = TRUE),
                                     k=cl,
                                     ini='random',
                                     dist='gower',
                                     cont=1:3,
